...
export const fetchDashboardData = async () => {
  ...
  const trendStart = getLastSevenDays()[0].date.toISOString();
  ...
  const [
    ...
    supabase
      .from("products")
      .select("product_id, created_at, is_active"),
    supabase
      .from("stock_balances")
      .select(`
        stock_balance_id,
    ...
    supabase.from("workers").select("worker_id"),
    supabase
    ...
    supabase
      ...
      .select("purchase_request_id, request_status, requested_date"),
  ]);

  const products = assertNoError(productsResult);
  const balances = assertNoError(balancesResult).map(normalizeStockBalance);
  const loans = assertNoError(loansResult);
  const workers = assertNoError(workersResult);
  const movements = assertNoError(movementsResult);
  const purchaseRequests = assertNoError(purchaseRequestsResult);

  const lowStockCount = balances.filter(
    (item) => item.availableQty > 0 && item.availableQty <= LOW_STOCK_THRESHOLD
  ).length;
  ...
  const overdueLoans = loans.filter((loan) => isOverdueLoan(loan, today)).length;
  const dueSoonLoans = loans.filter((loan) =>
    isDueSoonLoan(loan, today, nextSevenDays)
  ).length;
  ...
  return {
    ...
    chartData: buildStockTrend(movements),
  };
};
